...
build:
	...
	@docker run --rm --privileged -v $(PWD):/workspace --tmpfs /build:exec,dev,suid,rw mcc-os-dev bash -c "cp -a /workspace/* /build/ && cd /build && sudo lb config && sudo lb build && cp *.iso /workspace/ || true"
